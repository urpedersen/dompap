...
def test_energy_conservation(verbose=False):
    ...
    velocities = np.array(
        [[-0.3, -0.1, 0.1], [0.2, 0.1, 0], [-0.2, -0.2, 0.1], [0.3, 0.2, -0.1]],
        dtype=np.float64,
    )
    ...
    temperature_damping_time = np.float64(np.inf)
    ...
    for _ in range(100):
        ...
        positions, velocities = make_one_step_leap_frog(
            positions, velocities, forces, masses, time_step
        )
        ...
        data.append(
            [float(potential_energy), float(kinetic_energy), float(total_energy)]
        )
        if verbose:
            print(data[-1])
    ...
    assert (
        np.std(data, axis=0)[2] < 1e-2
    ), f"Std of Total Energy fluctuation: {np.std(data, axis=0)[2]}"
    ...
    for _ in range(100):
        forces = compute_gravitational_forces(positions, masses)
        positions, velocities, _ = make_one_step(
            positions,
            velocities,
            betas,
            forces,
